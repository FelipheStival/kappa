...
graficoRelatorioClasses = function(dados){
  ...
  dados = dados[,c('classe_1', 'classe_2', 'classe_3', 'classe_4', 'classe_5', 'classe_6')]
  dados = melt(dados) %>% group_by(variable) %>% summarise(value = sum(value))
...
graficoHistograma = function(dadosPeriodo, dadosSigla){
  ...
  grafico = ggplot(dadosPeriodo, aes(x =prod)) + 
    ...
    geom_vline(xintercept = sum(dadosSigla$prod), linetype = 2, color = "red", lwd = 1.2) +
    xlab('Produtividade') +
    ylab('Frequência') +
    theme_minimal()
...
graficoDistribuicao = function(dadosPeriodo, dadosSigla){
  ...
  dadosPlotPeriodo = dadosPeriodo[,c('classe_1', 'classe_2', 'classe_3', 'classe_4', 'classe_5', 'classe_6')]
  ...
  dadosPlotSigla = dadosSigla[,c('classe_1', 'classe_2', 'classe_3', 'classe_4', 'classe_5', 'classe_6', 'sigla_fiscal')]
  dadosPlotSigla = melt(dadosPlotSigla) %>% group_by(sigla_fiscal, variable) %>% summarise(value = sum(value)) %>% as.data.frame()
  ...
  dadosPlotSigla$subgrupo = dadosPlotSigla$sigla_fiscal
  
  dadosPlotSigla$sigla_fiscal = NULL
